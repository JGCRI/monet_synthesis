...
#TODO change this and the next chunk so that they stop if the files aren't there

load("R_data/processed_data.Rdata")

if(!"processed_data.Rdata" %in% list.files("R_data/")){
...
#not used
rm("pH_zones", "clay_zones", "clay_loc_sf_btm",
   "climate_zone_mapping", "pH_top_long", "pH_btm_long",
   "clay_top_long", "clay_btm_long")

```

### Load Pre-processed Data 2 of 2 (ditto)

```{r SoilGrids data, warning=FALSE, message=FALSE}

load("R_data/sg_data.Rdata")

if(!"sg_data.RData" %in% list.files("R_data/")){
...
#not used
rm("sg_clay_conus_btm",
   "sg_pH_monet_top_values",
...
```{r Plot SRDB and MONet data, cache = TRUE, include = FALSE, fig.dim=c(6,4)}

# read in site coordinates and data
# only the 1000 soils subset of MONet has respiration data currently
monet_rs <- read.csv("data/MONet/1000S_processed_L2_summary.csv")
# 1000 soils project metadata
metadata_monetRs <- read.csv("data/MONet/1000Soils_Metadata_Site_Mastersheet_v1.csv")
...
# Read-in and validate CONUS shapefile
...
# Define the target CRS (coordinate reference system) from the shapefile
...
rs_index_results <- combined_monet %>%
  ...
  mutate(
    respiration_rate = rs_index/2.06e-03,
    source = "MONet"
  )
...
converted_srdb <- combined_SRDB %>%
  mutate(
    Rs_daily_mg_per_g_soil = Rs_annual * 2.06e-05)
    # Conversion formula
  ) %>%
  na.omit() %>%
  filter(MAT > 8 & MAT < 16.5 & MAP > 200 & MAP < 1230) %>%
  #filtering to climate range of MONet
  slice_sample(n = 32) %>% #2x MONet sample size
  mutate(
...
soil_Rh_buffer %>%
  left_join(rs_index_results, by =  "Site_Code") %>%
  mutate(ymin = sg_mean - sg_sd,
         ymax = sg_mean + sg_sd)%>%
  ggplot(aes(x = respiration_rate, y = sg_mean))+
  geom_point()+
  geom_errorbar(aes(x = respiration_rate , ymin = ymin, ymax = ymax))+
...
```{r plot pH, fig.dim=c(6,9), fig.align='center', include = FALSE}
...
sg_clay_top_prj <- terra::rast("R_data/sg_clay_top_prj.tif") 
...
sg_clay_conus_top <- terra::project(sg_clay_top_prj, conus_valid)%>%project(crs(conus_valid))
# sg_clay_conus_btm <- raster::mask(sg_clay_btm_prj, conus_valid)%>%project(crs(conus_valid))

#p1 <- 
  ggplot()+
  tidyterra::geom_spatraster(data = sg_clay_conus_top/10) +
  geom_sf(data = conus_valid, color = "black", alpha = 0.3)+
