...
```{r Comparison of Soil Respiration relative to MAP and MAT, results = "hide", fig.dim=c(6,4), fig.align='center'}
...
-   **Molecular weight ratio**: 1 g C = 3.67 g CO2.

Using the equation:

\text{mg CO2 g⁻¹ soil day⁻¹} = \left( \frac{\text{g C m² yr⁻¹} \times 3.67}{365 \times 185,000} \right) \times 1,000
...
head(combined_data_mg_co2_c_g_soil_day)

# Linear regression model to test the effect of MAP and MAT on Rs_annual
model <- lm(Rs_annual ~ MAP + MAT, data = combined_data_mg_co2_c_g_soil_day)
summary(model)
...
plot_MAP <- ggplot(combined_data_mg_co2_c_g_soil_day, aes(x = MAP, y = Rs_mg_co2_g_24h)) +
...
plot_MAT <- ggplot(combined_data_mg_co2_c_g_soil_day, aes(x = MAT, y = Rs_mg_co2_g_24h)) +
  geom_point(color = myColors["comp_resp"], alpha = 0.5) +
...
```{r}
wide_monet <- combined_monet%>%
  filter(!is.na(respiration_mg_co2_c_g_soil_day_24hour))%>%
  mutate(Sample_Name = sub("_[A-Z]{3}$", "",Sample_Name))%>%
  pivot_longer(cols = 3:4, names_to = "Rs_level", values_to = "value")%>%
  unite(Rs_level, c(Core_Section, Rs_level), sep = "_")%>%
  pivot_wider(values_from = value, names_from = Rs_level)


MONet_index <- wide_monet%>%
  mutate(Rs_index = 0.5 * (( 0.65 * (TOP_respiration_mg_co2_c_g_soil_day_24hour + TOP_respiration_mg_co2_c_g_soil_day_96hour)) + 
                          0.35 * (BTM_respiration_mg_co2_c_g_soil_day_24hour + BTM_respiration_mg_co2_c_g_soil_day_96hour)))


ggplot(MONet_index, aes(x = MAP, y = index))+geom_point()

ggplot(MONet_index, aes(x = MAT, y = index))+geom_point()
...
# Compute linear models and summarize using select() inside group_by
lm_models_Rs_index <- MONet_index %>%
  summarise(
    model_MAT = list(lm(Rs_index ~ MAT, data = dplyr::select(., MAT, Rs_index))),
    model_MAP = list(lm(Rs_index ~ MAP, data = dplyr::select(., MAP, Rs_index)))
  )
...
p_monet_MAT_core_24h <- ggplot(MONet_index, aes(x = MAT, y = Rs_index)) +
  geom_point(color = myColors["MONet_resp"], alpha = 0.5) +
  geom_smooth(method = "lm", col = "black") +
  labs(title = "Effect of MAT on Respiration Index for Monet Data",
       x = "Mean Annual Temperature (MAT)",
       y = "Respiration (CO2 ppm in core sub-section)") +
  theme_bw() +
  theme(legend.position = "bottom")+
...
p_monet_MAP_core_24h <- ggplot(combined_monet, aes(x = MAP, y = respiration_mg_co2_c_g_soil_day_24hour, color = Core_Section)) +
  ...
  geom_smooth(method = "lm", col = "black") +
  facet_wrap(~Core_Section) +
  labs(title = "Effect of MAP on Respiration Index for Monet Data",
       x = "Mean Annual Precipitation (MAP)",
       y = "Respiration (CO2 ppm in core sub-section)") +
  theme_bw() +
  theme(legend.position = "bottom")
...
if(!"sg_data.RData" %in% list.files()){
  ...
  save(sg_clay_conus_top, sg_clay_conus_btm,
       clay_top_long, clay_btm_long, sg_clay_top_prj,
...
```{r plot comparisons across zones}
...
```{r plot comparisons, fig.dim=c(10,10)}
plot_data_clay_top%>%
   mutate(Experiment = as.factor(if_else(source == "MONet", "MONet_clay", "comp_clay")))%>%
 ...
 plot_data_pH_top%>%
   ...
   labs(
     title = "MONet and Soilgrids pH Content by Climate Zone (Top sample)",
     x = "pH",
     y = "Climate Zone"
   )+
  scale_y_discrete(labels = label_wrap_gen(30))
  ...
  plot_data_pH_btm%>%
   ...
   scale_color_manual(name = "Experiment", values = myColors, labels = c("comp_pH" = "Soilgrids", "MONet_pH" = "MONet"))+
   labs(
     title = "MONet and Soilgrids pH Content by Climate Zone (Bottom sample)",
     x = "pH",
     y = "Climate Zone"
   )+
  scale_y_discrete(labels = label_wrap_gen(30))
...
```{r random sample plottting, fig.dim=c(10,10) }
...
clay_random_samples <- rbind(
  clay_sample_1%>%
    mutate(source = "Soilgrids_sample1"),
  clay_sample_2%>%
    filter(source == "Soilgrids")%>%
    mutate(source = "Soilgrids_sample2"),
  clay_sample_3%>%
    filter(source == "Soilgrids")%>%
    mutate(source = "Soilgrids_sample3")
)
