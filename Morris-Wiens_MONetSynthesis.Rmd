...
ggplot(MONet_index, aes(x = MAP, y = Rs_index))+geom_point()

ggplot(MONet_index, aes(x = MAT, y = Rs_index))+geom_point()
...
# Display model summaries for 'MAT' and 'MAP'
for (i in 1:nrow(lm_models_Rs_index)) {
  cat("Model MAT Summary:\n")
  print(summary(lm_models_Rs_index$model_MAT[[i]]))
  cat("\nModel MAP Summary:\n")
  print(summary(lm_models_Rs_index$model_MAP[[i]]))
  cat("\n----------------------\n")
}
...
if(!"sg_data.RData" %in% list.files()){
  ...
  print("Load SoilGrids data by running MONet_data_processing.Rmd")
...
clay_MONet_sg_top2 <- process_soil_data(
  type = "Clay_percent",
  section = "TOP",
  monet_df = clay_zones,
  sg_df = clay_top_long,
...
lay_MONet_sg_btm <- process_soil_data(
  type = "Clay_percent",
  core_section = "BTM",
  monet_df = clay_zones,
  sg_df = clay_btm_long,
...
sg_random_sample <- function(clay_MONet_sg_top, sample_sizes_sg_top, seeds){
...
clay_samples%>%
  ...
  scale_y_discrete(labels = label_wrap_gen(30))+
  scale_fill_manual(name = "Experiment", values = sample_colors_clay)+
  scale_color_manual(name = "Experiment", values = sample_colors_clay)
...
# TODO make compatible with violins in sectionm above
# generate_violin_plots(plot_data = clay_samples%>%
#    left_join(sample_sizes_sg_top, by = c("Climate")),
#                       value_col = "Clay_percent", 
#                       source_mapping = c("comp_pH" = "Soilgrids", "MONet_pH" = "MONet"),
#                       title = "MONet and Soilgrids pH by Climate Zone (Bottom sample)", 
