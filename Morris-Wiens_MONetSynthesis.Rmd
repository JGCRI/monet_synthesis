...
## Load in MONet and Soilgrids data

```{r}
# Load in processed data from download_and_preprocessing.R
load("processed_data.Rdata")

if(!"processed_data.Rdata" %in% list.files()){
  
  print("Load processed MONet data by running MONetDataPreprocessing.R")
...
# 2. Assessemnt within Koppen Climate Zones
...
if(!"sg_data.RData" %in% list.files()){
  ...
  print("Load SoilGrids data by running MONetDataPreprocessing.R")
...
```{r MONet not in Soilgrids, fig.dim=c(6,5), fig.align='center'}
...
```{r Plot histograms, fig.dim=c(8,6), fig.align='center'}
...
```{r Spatial comparison, fig.dim = c(8,8), fig.align='center'}
