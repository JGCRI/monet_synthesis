...
ggplot()+
  geom_sf(data = conus_basins, aes(fill = glu_nm))+
  geom_sf(data= clay_loc_sf_btm)
...
unique(clay_basins$glu_nm)

unique(GCAM_clay_usa$GCAM_GLU_name)


clay_basin_summary <- clay_basins%>%
  data.frame()%>%
  group_by(core_section, glu_id, glu_nm)%>%
  summarize(Clay_percent_mean = mean(Clay_percent), monet_sd = sd(Clay_percent), monet_min = min(Clay_percent), monet_max = max(Clay_percent), monet_n = n())
